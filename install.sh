...
if which curl >/dev/null; then
    ...
    LATEST=$(wget -qO- https://api.github.com/repos/cryon-io/alis-cli/releases/latest | grep tag_name | sed 's/  "tag_name": "//g' | sed 's/",//g')
fi

# install ami
echo "Downloading ami setup script..."
if ! "$@" https://raw.githubusercontent.com/cryon-io/ami/master/install.sh; then
...
if "$@" "https://github.com/cryon-io/alis-cli/releases/download/$LATEST/alis-cli-unix-$(uname -p).zip" &&
